...
class Worker
{
     ...
     */
    public function __construct(Manager $manager, Dispatcher $events, ErrorHandler $exceptions)
    {
        $this->events = $events;
    ...
    protected function registerTimeoutHandler($job, WorkerOptions $options)
    {
        if ($this->supportsAsyncSignals()) {
            ...
            pcntl_signal(SIGALRM, function () use($me) {
    ...
    protected function daemonShouldRun(WorkerOptions $options, $connectionName, $queue)
    {
        return ! (($this->manager->isDownForMaintenance() && ! $options->force) ||
            $this->paused ||
            $this->events->until(Looping::CLASSNAME) === false);
    }
    ...
    protected function failJob($connectionName, $job, $e)
    {
        ...
        try {
            ...
            $this->events->dispatch(new JobFailed($connectionName, $job, $e ?: new ManuallyFailedException));
        }
        $this->events->dispatch(new JobFailed($connectionName, $job, $e ?: new ManuallyFailedException));
    }
    ...
    protected function getTimestampOfLastQueueRestart()
    {
        if ($this->cache) {
            ...
            return $this->cache->get('pp_queue_restart');
        }
    ...
    protected function listenForSignals()
    {
        if ($this->supportsAsyncSignals()) {
            ...
            pcntl_signal(SIGTERM, function () use($me) {

                $me->shouldQuit = true;
            });

            pcntl_signal(SIGUSR2, function () use($me) {

                $me->paused = true;
            });

            pcntl_signal(SIGCONT, function () use($me) {
