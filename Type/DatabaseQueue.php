...
class DatabaseQueue extends Queue implements QueueInterface
{
    ...
    public function bulk($jobs, $data = '', $queue = null)
    {
        ...
        return $this->database->saveInBulk(Collection::make((array)$jobs)->map(
            ...
            function ($job) use ($queue, $data, $availableAt, $self) {

                return $self->buildDatabaseRecord($queue, $self->createPayload($job, $data), $availableAt);
            }
    ...
    {
        return $this->database->createRecord(array(
            'queue'        => $queue,
            'payload'      => $payload,
            'attempts'     => $attempts,
            'reserved_at'  => null,
